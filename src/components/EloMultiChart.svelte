<script lang="ts">
	...
	const TW = 56,
		TH = 26,
		TR = 5;
	function tipX(x: number) {
		return x + TW + 10 > W ? x - TW - 6 : x + 6;
	}
	...
	>
		...
		{#each dates as date, i}
			<text
				...
				font-size="9"
				fill="#b3b1ad"
				font-family="ui-sans-serif, system-ui, sans-serif">{fmtDate(date)}</text
			>
		{/each}
		...
		{#if hoveredDot}
			...
			<g style="pointer-events: none;">
				...
				<text
					...
					font-weight="600"
					fill="#e6e1cf"
					font-family="ui-monospace, monospace">{hoveredDot.elo}</text
				>
			</g>
		{/if}
	</svg>
	...
	<div class="mt-3 flex flex-wrap gap-x-5 gap-y-1.5">
		{#each colored as player}
			...
			>
				...
				></span>
				<span
					class="transition-colors {hoveredPlayer === player.player_id
						? 'text-white'
						: 'text-zinc-400'}">{player.name}</span
				>
			</button>
		{/each}
	</div>
