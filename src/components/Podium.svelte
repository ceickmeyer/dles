		...
		{#each [1, 0, 2] as pos}
			...
			{#if player}
				<div class="flex flex-col items-center gap-2">
					...
					<span class="max-w-32 text-center text-sm leading-tight font-semibold text-white">
						{player.player_name}
					</span>
					<div
						class="flex w-24 items-end justify-center overflow-hidden rounded-t-lg text-sm font-bold"
						style:background-color={colors[pos]}
						style:color="var(--color-ayu-bg)"
						style:height={heights[pos]}
						style:transform-origin="bottom"
						style:animation={animate
							? `bar-rise 0.7s cubic-bezier(0.34,1.56,0.64,1) ${delays[pos]} both`
							: 'none'}
					>
						<div class="py-2 text-center text-xs leading-relaxed">
							<div>{player.gold}🥇</div>
