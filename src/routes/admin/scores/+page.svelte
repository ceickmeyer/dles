...
<div class="space-y-6">
	<div>
		...
		<p class="mt-0.5 text-sm text-ayu-muted">
			Click a session to view and delete individual scores.
		</p>
	</div>
		...
		<div class="overflow-hidden rounded-xl border border-ayu-border">
			<table class="w-full text-sm">
				...
				<tbody>
					{#each sessions as s}
						<tr
							class="border-b border-ayu-border bg-ayu-surface transition last:border-0 hover:bg-ayu-surface2"
						>
							<td class="px-4 py-3 font-medium">
								<a href="/admin/sessions/{s.id}" class="text-white transition hover:text-ayu-gold"
									>{s.name}</a
								>
							</td>
