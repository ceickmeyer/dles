<script lang="ts">
	...
	async function save() {
		...
		if (needsParser) {
			error = 'Select a share parser for "paste & parse" mode.';
			return;
		}
		if (needsMaxScore) {
			error = 'Set a max score of 1–12 for "buttons" mode.';
			return;
		}
		if (shareParser === 'custom' && !shareRegex.trim()) {
			error = 'Enter a regex pattern.';
			return;
		}
		saving = true;
		error = '';
	...
	<div class="flex items-start gap-6">
		...
		<div class="min-w-0 flex-1 space-y-5">
			...
			<div class="space-y-4 rounded-xl border border-ayu-border bg-ayu-surface p-5">
				<p class="text-xs font-semibold tracking-wider text-ayu-muted uppercase">Basics</p>
			...
			<div class="space-y-4 rounded-xl border border-ayu-border bg-ayu-surface p-5">
				<p class="text-xs font-semibold tracking-wider text-ayu-muted uppercase">
					How players submit their score
				</p>

				<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
					{#each [{ value: 'auto', label: 'Auto', desc: 'Smart defaults' }, { value: 'buttons', label: 'Buttons only', desc: '1–N quick pick' }, { value: 'parser', label: 'Paste & parse', desc: 'Share text only' }, { value: 'manual', label: 'Manual entry', desc: 'Type a number' }] as opt}
						<label
						...
						>
							...
							<p
								class="text-sm font-semibold {inputMode === opt.value
									? 'text-ayu-gold'
									: 'text-white'}"
							>
								{opt.label}
							</p>
							<p class="mt-0.5 text-xs text-ayu-muted">{opt.desc}</p>
						</label>
					{/each}
				</div>
						...
						<div class="space-y-3 rounded-lg border border-ayu-border bg-ayu-bg p-4">
							...
							<div>
								...
								<p class="mt-1 text-xs text-ayu-muted">
									Use a capture group <code class="text-ayu-gold">(\d+)</code> to extract the number.
								</p>
							</div>
							<div>
					...
					{#if inputMode === 'parser'}
						<div>
							<label class="mb-1.5 block text-sm font-medium text-zinc-300" for="max-display">
								Max score <span class="text-xs text-ayu-muted"
									>(optional — used in share text as "X/max")</span
								>
							</label>
							<input
								id="max-display"
			...
			<div class="rounded-xl border border-ayu-border bg-ayu-surface p-5">
				<p class="mb-3 text-xs font-semibold tracking-wider text-ayu-muted uppercase">Options</p>
		...
		<!-- Live Preview -->
		<div class="sticky top-6 w-72 shrink-0 space-y-2">
			<p class="text-xs font-semibold tracking-wider text-ayu-muted uppercase">
				Preview — fully interactive
			</p>
			<LobbyCard game={previewGame} preview={true} />
			<p class="text-xs text-ayu-muted">
				Nothing is saved. Use "test again" after submitting to reset.
			</p>
		</div>
	</div>
</div>
