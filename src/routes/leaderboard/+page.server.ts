...
import {
	...
	LEADERBOARD_MIN_DAYS as MIN_DAYS
} from '$lib/scoring';
...
export const load: PageServerLoad = async () => {
	...
	const gameData = new Map<
		string,
		GameMeta & { playerScores: Map<string, { name: string; vals: number[] }> }
	>();
	...
	const perGame = [...gameData.values()]
		.flatMap(({ playerScores, ...meta }) => {
	...
	for (const s of allScores) {
		if (!playerNames.has(s.player_id))
			playerNames.set(
				s.player_id,
				displayName(s.player as { name: string; alias?: string | null })
			);
	}
	...
	const qualified = (eloRows ?? [])
		...
		.map((r) => ({
			...
			sessions: r.sessions,
			history: r.history,
			name: playerNames.get(r.player_id) ?? '?'
		}))
