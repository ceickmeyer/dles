...
<div class="space-y-8">
	...
	{:else}
		{#if tally.length >= 2}
			<div class="rounded-xl border border-ayu-border bg-ayu-surface p-6">
				<h2 class="mb-4 text-xs font-semibold tracking-wider text-ayu-muted uppercase">
					Final Standings
				</h2>
				<Podium {tally} />
			</div>
	...
	<!-- Per-game breakdowns -->
	<div class="space-y-3">
		<h2 class="text-xs font-semibold tracking-wider text-ayu-muted uppercase">Game Breakdowns</h2>
					...
					<div class="space-y-1">
						{#each scores as ranked}
							...
							>
								...
								<span class="font-mono font-semibold text-ayu-gold"
									>{formatScore(ranked.raw_score, game)}</span
								>
							</div>
						{/each}
					</div>
